...
def parse_args():
    ...
    parser.add_argument('--interval_for_train', type=int, default=0,
                        help="select training frames based on intervals")
...
def run_colmap(args):
    ...
    do_system(
        f"colmap {args.colmap_matcher}_matcher --SiftMatching.guided_matching {flag_EAS} --database_path {db}")
    try:
...
if __name__ == "__main__":
    args = parse_args()
    # args.images = '/home/ilab/dzm/GitHub/nerf_for_ipm/data_in/video_kb714/images'
    # args.run_colmap = False
    args.interval_for_train = -1
    args.hold = -1
    ...
    def write_json_1(filename, frames):
        for f in frames:
    ...
    write_json_1('transforms_all.json', frames)
    N = len(frames)
    print('#####################')
    print(N)
    ...
    #     write_json('transforms_all.json', frames)

    # elif args.interval_for_train > 0:
    #     select_ids = np.arange(0, N, args.interval_for_train)
    #     select_frames = [f for i, f in enumerate(frames) if i in select_ids]
    #     write_json('transforms.json', select_frames)
