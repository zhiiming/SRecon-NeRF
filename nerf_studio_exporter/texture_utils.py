...
def export_textured_mesh(
    ...
    # CONSOLE.print(f"camera_ray_bundle.shape: {camera_ray_bundle.shape}", justify="center")
    ...
    with torch.no_grad():
        ...
        for idx in range(camera_ray_bundle.shape[0]):
            # CONSOLE.print(f"camera_ray_bundle[idx].shape: {camera_ray_bundle[idx].shape}", justify="center")
            sub_outputs = pipeline.model.get_outputs(camera_ray_bundle[idx])
            # CONSOLE.print(f"sub_outputs.shape: {sub_outputs.shape}", justify="center")
            sub_rgb = sub_outputs["semantics_colormap"]
            # CONSOLE.print(f"sub_rgb: {sub_rgb}", justify="center")
            # CONSOLE.print(f"sub_rgb.shape: {sub_rgb.shape}", justify="center")
            sub_rgb = sub_rgb[None, :]
            # CONSOLE.print(f"sub_rgb.shape: {sub_rgb.shape}", justify="center")
            rgb_list.append(sub_rgb)
            # torch.cat((rgb, torch.ones_like(rgb[..., :1])), dim=-1)
        # outputs = pipeline.model.get_outputs(camera_ray_bundle)
    ...
    CONSOLE.print(f"rgb.shape: {rgb.shape}", justify="center")
    # rgb = pipeline.model.get_rgba_image(outputs, "semantics_colormap")[..., :3]
    # rgb = pipeline.model.get_outputs(outputs)["semantics_colormap"]
    # models/semantic_nerfw.py, line 179
